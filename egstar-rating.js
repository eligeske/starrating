// JavaScript Document(function($){
		...
		(function($) {	
			...
			function seteditbox(egr, cont, val){
				var wd = egr.children('.eg-rating').width()
				+ egr.children('.eg-rtdash').width()
				 + 27 + (rating.smstarwidth*rating.maxstars);
				if(rating.allowedit){ wd = wd+ egr.children('.eg-edit').width(); }else{ wd = wd - 5; }
				cont.animate({ width: wd }, rating.a_speed, editfade(egr, val, cont) );
				
			}			
			function editfade(egr, val, cont){
				if(rating.allowedit){ egr.children('.eg-edit').fadeIn(); }
				if(typeof(rating.callback)==='function'){
					input = cont.next('input');
					val = { rating: val, input: input }
			...
			function rate(){
				...
				smstr.width(smwd); 
				egr.html(val);seteditbox(egr.parent(), $par.parent(), val);
			...
			function edit(){				
				$par = $(this).parent();$par.hide();$par.prev().show();$par.prev().prev().show();
				$par.parent().animate({ width: rating.starwidth*rating.maxstars},rating.a_speed);
			}
			function buildFrag(){			
				...
				var cont = $("<div>",{ "class": "eg-rtcont", width: contwd+"px" });
				var rtd =  $("<div>",{ "class": "eg-rated" });
				var ul = $("<ul>"); 						
				var li = $("<li>", { html: "&nbsp;", mouseover: roll, mouseout: out, click: rate, width: liwd+"px" });					
				var m = (rating.halfstars)?2*rating.maxstars:rating.maxstars;
				for($i=0;$i<m;$i++){ li.clone(true).appendTo(ul); }				
				ul.appendTo($("<div>",{ "class": "eg-rtbtns" }).appendTo(cont));
				$("<div>",{ "class": "eg-on", html: "&nbsp;" }).appendTo(cont);
				$("<div>",{ "class": "eg-rating", text: "0" }).appendTo(rtd);
				$("<div>", { "class": "eg-rtdash", text: "-" } ).appendTo(rtd);
				$("<div>", { "class": "eg-smallstars", html: "<div>&nbsp;</div>" }).appendTo(rtd);
				if(rating.allowedit){
				$("<div>", { "class": "eg-edit", text: "edit", click: edit }).appendTo(rtd);}
				rtd.appendTo(cont);
				return cont; 			
			}
